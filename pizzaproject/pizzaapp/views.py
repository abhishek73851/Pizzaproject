...
def makepayment(request):
    if request.user.is_authenticated:
        ...
        for x in orders:
            total_price += x.sno.pizza_price * x.qty
            oid=x.order_id
        client = razorpay.Client(auth=("rzp_test_KRxDn3MQNJ1zzJ", "VjRoumSsH8oLZSSLGZ8MAM4S"))
        data = { "amount": total_price*100, "currency": "INR", "receipt": str(oid) }
        payment = client.order.create(data=data)
        # print(payment)
        subject =f"Pizza payment done for Order Id: {oid}"
        message = f"Hi{user},Thank you for your order \n  Total Amount Paid ={total_price}Rs/-"
